...
impl BifrostManager {
    pub async fn new() -> Self {
        let binary_manager = Arc::new(RwLock::new(
            BinaryManager::new().await.unwrap_or_else(|e| {
        ...
        let available_port = Self::find_available_port("127.0.0.1", 3003).await
    ...
    async fn find_local_bifrost_binary(_binary_manager: &Arc<RwLock<BinaryManager>>) -> Option<PathBuf> {
        // Determine the correct binary name based on platform
        let binary_name = if cfg!(windows) { "bifrost-http.exe" } else { "bifrost-http" };
        let platform_specific_name = format!("{}-{}", binary_name.trim_end_matches(".exe"), Self::get_platform_target());
        ...
        for name in &binary_names {
            ...
            if local_binary_path.exists() && local_binary_path.is_file() {
                println!("Found locally-built Bifrost binary at: {:?}", local_binary_path);
            ...
            if cwd_binary_path.exists() && cwd_binary_path.is_file() {
                println!("Found Bifrost binary in src-tauri directory: {:?}", cwd_binary_path);
        ...
        println!("  - binaries/bifrost-http-{} (platform-specific)", Self::get_platform_target());
        None
    }
        ...
        match Command::new(binary_path).arg("--version").output().await {
            Ok(output) => {
                if output.status.success() {
                    ...
                    println!("Bifrost binary version check passed: {}", version_str.trim());
                    return true;
                }
            }
            Err(e) => {
    ...
    pub async fn start(&mut self) -> MindLinkResult<()> {
        ...
        if let Some(monitor) = get_process_monitor() {
            ...
            if let Err(e) = monitor.register_process(
                "bifrost".to_string(),
        ...
        if std::env::var("BIFROST_LOG_LEVEL").unwrap_or_default().is_empty() {
                ...
                Ok(None) => {
                    if let Some(logger) = get_logger() {
                        let entry = LogEntry::new(
                            LogLevel::Info,
                            LogCategory::System,
                            format!("Bifrost LLM Router started successfully on {}:{}", self.host, self.port),
                        ).with_component("BifrostManager");
                        logger.log(entry);
                    }
                }
    ...
    pub async fn get_installation_info(&self) -> (bool, Option<PathBuf>, Option<String>) {
        ...
        let status_message = if binary_available {
            ...
            Some("Locally-built Bifrost binary not found - build system needs to be run".to_string())
        };
