...
pub enum ProcessEvent {
    ...
    Stopped { process_id: String, exit_code: Option<i32> },
    Crashed { process_id: String, error: String },
    OutputReceived { process_id: String, output_type: String, content: String },
    RestartAttempted { process_id: String, attempt: u32 },
    RestartLimitReached { process_id: String },
    HealthCheckFailed { process_id: String, error: String },
}
        ...
        let should_restart = {
            ...
            if let Some(info) = processes.get_mut(process_id) {
                ...
                if info.restart_count >= config.max_restart_attempts {
                    ...
                    return Err(MindLinkError::ProcessMonitoring {
                        message: format!("Restart limit reached ({} attempts)", config.max_restart_attempts),
