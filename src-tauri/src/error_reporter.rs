...
impl ErrorReporter {
    ...
    pub async fn report_error(
        ...
        if let Some(logger) = get_logger() {
            logger.log_error(&context.component, &error, context.correlation_id.as_deref());
        }
    ...
    async fn notify_user(
        ...
        let should_show_dialog = self.config.show_user_dialogs && 
            self.should_show_dialog(&report.error, context);
        
        if should_show_dialog {
            // Show blocking dialog for critical errors
            let dialog_result = DialogManager::show_error(
                app_handle,
                &report.error,
            ...
            match operation() {
                Ok(result) => {
                    ...
                    if let Some(error) = last_error {
                        self.mark_resolved(&error, &format!("Resolved on attempt {}", attempt)).await;
                    }
                    return Ok(result);
                }
                Err(e) => {
                    ...
                    error_context.additional_info.insert(
                        "attempt".to_string(),
                        attempt.to_string(),
                    );
                    error_context.additional_info.insert(
                    ...
                    let report = self.report_error(app_handle, mindlink_error.clone(), error_context).await;
                       ...
                       mindlink_error.is_recoverable() {
                        ...
                        if let Some(logger) = get_logger() {
                            let entry = LogEntry::new(
                                LogLevel::Info,
                                LogCategory::System,
                                format!("Retrying operation (attempt {} of {})", attempt + 1, max_attempts),
                            ).with_component("ErrorReporter");
                            logger.log(entry);
                        }
    ...
    pub async fn get_error_statistics(&self) -> ErrorStatistics {
        ...
        for report in history.iter() {
            *by_component.entry(report.context.component.clone()).or_insert(0) += 1;
        }
