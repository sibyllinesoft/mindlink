    ...
    fn get_app_data_dir() -> Result<PathBuf> {
        ...
        #[cfg(target_os = "macos")]
        {
            if let Some(home) = std::env::var_os("HOME") {
                return Ok(PathBuf::from(home).join("Library").join("Application Support").join(app_name));
            }
        }
    ...
    pub async fn build_bifrost(&self) -> MindLinkResult<PathBuf> {
        ...
        if !build_script.exists() {
            return Err(MindLinkError::BinaryExecution {
                message: "Build script not found. Make sure you're running from the project root.".to_string(),
        ...
        log_info!("BinaryManager", format!("Found build script at: {:?}", build_script));
        
        // Run the build script with real-time logging
        let script_path = build_script.to_str()
        ...
        log_info!("BinaryManager", format!("Executing build script: {}", script_path));
        ...
        let exit_status = child.wait().await.map_err(|e| MindLinkError::BinaryExecution {
        ...
        if !exit_status.success() {
            let error_msg = format!("Build script failed with exit code: {:?}", exit_status.code());
            log_error!("BinaryManager", MindLinkError::BinaryExecution {
        ...
        let binary_path = self.get_local_bifrost_path()
            .ok_or_else(|| MindLinkError::BinaryExecution {
        ...
        log_info!("BinaryManager", format!("Successfully built and verified Bifrost binary at: {:?}", binary_path));
        
        Ok(binary_path)
    }
    
    /// Verify binary integrity and generate checksum
    async fn verify_binary_integrity(&self, binary_path: &std::path::Path) -> MindLinkResult<String> {
        use tokio::fs::File;
        ...
        let mut file = File::open(binary_path).await.map_err(|e| {
            MindLinkError::FileSystem {
        ...
        log_info!("BinaryManager", format!("Binary verified - Size: {} bytes, SHA256: {}", 
                 metadata.len(), checksum));
...
"#
    ...
    pub fn get_binary_path(&self, binary_name: &str) -> Option<PathBuf> {
        ...
        let possible_paths = vec![
            ...
            binary_dir.join("node_modules").join(".bin").join(binary_name),
            binary_dir.join("node_modules").join(".bin").join(format!("{}.exe", binary_name)),
        ];
    ...
    pub fn get_local_bifrost_path(&self) -> Option<PathBuf> {
        ...
        let binary_name = if cfg!(windows) { "bifrost-http.exe" } else { "bifrost-http" };
    ...
    pub async fn verify_binary(&self, binary_path: &Path) -> Result<bool> {
        ...
        let filename = binary_path.file_name().and_then(|n| n.to_str()).unwrap_or("");
        if filename == "bifrost-http" || filename == "bifrost-http.exe" || filename.starts_with("bifrost-http-") {
            ...
            match TokioCommand::new(binary_path).arg("--version").output().await {
    ...
    pub async fn ensure_cloudflared(&self) -> Result<PathBuf> {
        ...
        if let Ok(output) = TokioCommand::new("cloudflared").arg("--version").output().await {
    ...
    async fn download_cloudflared(&self) -> Result<PathBuf> {
        ...
        if download_url.ends_with(".tgz") {
            ...
            return Err(anyhow!("Compressed downloads not yet supported. Please install cloudflared manually."));
        } else {
