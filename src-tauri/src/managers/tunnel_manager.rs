...
impl TunnelManager {
    ...
    async fn create_quick_tunnel(&mut self) -> Result<String> {
        ...
        let mut child = Command::new(&cloudflared_path)
            .args(&[
                "tunnel",
                "--url", &format!("http://localhost:{}", self.local_port),
                "--no-autoupdate"
            ])
...
mod tests {
    ...
    async fn test_tunnel_manager_creation() {
        ...
        assert!(!manager.is_connected().await, "Manager should start disconnected");
        assert_eq!(manager.get_current_url().await, None, "Should have no URL initially");
    }
